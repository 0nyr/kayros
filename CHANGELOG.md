...
- **`max_perturbation_pct`** (default 0.0 = inert): opt-in n-scaling of the perturbation ceiling, `max_removals = max(max_perturbations, ceil(pct · n))`. Any non-zero value shifts the removal-target draw span and thus the rng stream, so the default changes nothing.

### Changed

- **BREAKING (long Duration runs at defaults): the work-based restart is ON by default** (`restart_no_improvement_work = 250_000_000`, roughly a few hundred seconds of stall on one modern core at any instance size; the flat `restart_no_improvement = 20000` remains as a backstop, whichever fires first wins). Rationale: the flat default was effectively dead code on realistic budgets at n ≥ 500 (it never fired once across two Blauth2024 campaign nights), so stalled searches never restarted. Duration streams in runs long enough to accumulate a 250M-unit stall window now diverge from 1.1.x at default parameters; pass `restart_no_improvement_work=0` to recover bitwise 1.1.x trajectories (the cross-version hex gate runs exactly that leg).
