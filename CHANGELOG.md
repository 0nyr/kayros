...
- **Work-based triggers** (`fd_period_work`, `restart_no_improvement_work`, both default off): thresholds counted in LS work units (candidate pricings, reported as `Solution.work_units` alongside the new `Solution.restarts`). A deterministic wall-time proxy that self-scales with instance size, where flat iteration counts fit one size class only (Blauth2024 iteration velocity spans ~1100 it/s at n=10 to ~0.2 it/s at n=2000, so the stock `restart_no_improvement=20000` never fired at n ≥ 500 in a 3-5 h run).
